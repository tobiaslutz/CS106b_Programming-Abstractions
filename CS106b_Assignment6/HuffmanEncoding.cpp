...
Node* buildEncodingTree(Map<ext_char, int>& frequencies) {
    ...
    for (ext_char ch : frequencies)   {
        ...
        leaf->zero = NULL;
        leaf->one = NULL;
        nodes.enqueue(leaf, frequencies.get(ch));
    }
    
    while (nodes.size() > 1)    {
        Node * zero = nodes.peek();
        nodes.dequeue();
        Node * one = nodes.peek();
        nodes.dequeue();
        
        Node * interNode = new Node;
        interNode->character = NOT_A_CHAR;
        interNode->weight = zero->weight + one->weight;
        interNode->zero = zero;
        interNode->one = one;
        
        nodes.enqueue(interNode, interNode->weight);
    }
...
void encodeFile(istream& infile, Node* encodingTree, obstream& outfile) {
    ...
    generateCodebook(encodingTree, codeword, codebook);
    while ( (ch = infile.get()) != EOF)
    {
        writeToFile(codebook.get(ch), outfile);
    }
    
    writeToFile(codebook.get(PSEUDO_EOF), outfile);
...
void decodeFile(ibstream& infile, Node* encodingTree, ostream& file) {
    ...
    for (int i = 0; i < infile.size() * 8; i++) {
        ...
        if (currentNode->zero == NULL && currentNode->one == NULL)    {
            if(currentNode->character == PSEUDO_EOF)
                return;
            file.put(currentNode->character);
            currentNode = root;
        }
    }
}
...
void generateCodebook(Node* encodingTree, string codeword, Map<int, string> & codebook)   {
    ...
    if(encodingTree->zero != NULL)  {
        c += "0";
        trim = true;
        generateCodebook(encodingTree->zero, c, codebook);
    }
    ...
    if(encodingTree->one != NULL)  {
        ...
        c += "1";
        generateCodebook(encodingTree->one, c, codebook);
    }
    
    if (encodingTree->zero == NULL && encodingTree->one == NULL)
        codebook.put(encodingTree->character, c);
    
}

void writeToFile(string codeword, obstream& outfile)   {
